...
const DEFENSE_ICONS: Record<string, React.ReactElement> = {
...
type StimulusType = 'color' | 'arrow' | 'number' | 'defense';
...
export default function CognitiveTraining() {
    const [selectedColors, setSelectedColors] = useState<string[]>(COLOR_OPTIONS.slice());
    const [selectedArrows, setSelectedArrows] = useState<string[]>(ARROW_OPTIONS.slice());
    const [selectedNumbers, setSelectedNumbers] = useState<string[]>(NUMBER_OPTIONS.slice());
    const [selectedDefenses, setSelectedDefenses] = useState<string[]>([]);
    const [durationSec, setDurationSec] = useState<number>(30);
    const [elementDurationMinMs, setElementDurationMinMs] = useState<number>(600);
    const [elementDurationMaxMs, setElementDurationMaxMs] = useState<number>(1000);
    ...
    function buildStimuli(): Stimulus[] {
        const list: Stimulus[] = [];
    ...
    function pickOne(list: Stimulus[]): Stimulus | null {
    ...
    function toggleItem(setter: React.Dispatch<React.SetStateAction<string[]>>, list: string[], item: string) {
    ...
    function setAllColors(select: boolean) {
        setSelectedColors(select ? COLOR_OPTIONS.slice() : []);
    }

    function setAllArrows(select: boolean) {
        setSelectedArrows(select ? ARROW_OPTIONS.slice() : []);
    }

    function setAllNumbers(select: boolean) {
        setSelectedNumbers(select ? NUMBER_OPTIONS.slice() : []);
    }

    function setAllDefenses(select: boolean) {
        setSelectedDefenses(select ? DEFENSE_OPTIONS.slice() : []);
    }

    function setAllGlobal(select: boolean) {
        setAllColors(select);
        setAllArrows(select);
    ...
    return (
        <Box sx={{ p: 3, minHeight: '60vh' }}>
            <Box sx={{ typography: 'h5', mb: 2 }}>Cognitive training</Box>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                    <Paper sx={{ p: 2, width: '100%', maxWidth: 720 }}>
                        ...
                        <Box sx={{ mt: 2 }}>
                            ...
                            <TextField type="number" size="small" value={durationSec} onChange={(e) => setDurationSec(Number(e.target.value))} sx={{ mt: 1, width: 140 }}  />
                        </Box>

                        <Box sx={{ mt: 2, display: 'flex', gap: 2, alignItems: 'center' }}>
                            <Box>
                                <Typography variant="body2">Element duration min (ms)</Typography>
                                <TextField type="number" size="small" value={elementDurationMinMs} onChange={(e) => setElementDurationMinMs(Number(e.target.value))} sx={{ mt: 1, width: 140 }} />
                            </Box>
                            <Box>
                                <Typography variant="body2">Element duration max (ms)</Typography>
                                <TextField type="number" size="small" value={elementDurationMaxMs} onChange={(e) => setElementDurationMaxMs(Number(e.target.value))} sx={{ mt: 1, width: 140 }} />
                            </Box>
                        </Box>
