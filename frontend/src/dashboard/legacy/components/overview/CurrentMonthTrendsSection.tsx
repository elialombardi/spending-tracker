                            ...
                            <Box sx={{ display: 'grid', gap: 1.25 }}>
                                {selectedWeekTransactions.map((transaction) => (
                                    ...
                                    >
                                        ...
                                        <Button
                                            variant={transaction?.isRelatedToCycle ? 'outlined' : 'contained'}
                                            size="small"
                                            disabled={!canWrite || isBusy || !transaction.category}
                                            sx={{ mt: 1.5, alignSelf: 'flex-start' }}
                                            onClick={() => onSetCycleIncomeTransactionRelation(transaction, !transaction?.isRelatedToCycle)}
                                        >
                                            {transaction?.isRelatedToCycle ? 'Mark as not related to cycle' : 'Count in cycle budget'}
                                        </Button>
                                        {!canWrite ? (
